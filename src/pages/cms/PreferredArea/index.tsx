...
import {addPreferredArea, queryPreferredAreaList, removePreferredArea, updatePreferredArea, updatePreferredAreaStatus} from './service';
 ...
 * @param fields
 */
const handleAdd = async (fields: PreferredAreaListItem) => {
  const hide = message.loading('正在添加');
  try {
    await addPreferredArea({...fields});
    hide();
 ...
 * @param fields
 */
const handleUpdate = async (fields: PreferredAreaListItem) => {
  const hide = message.loading('正在更新');
  try {
    await updatePreferredArea(fields);
    hide();
...
const handleStatus = async (ids: number[], status: number) => {
  ...
  try {
    await updatePreferredAreaStatus({ids, status});
    hide();
...
return (
    <PageContainer>
      <ProTable<PreferredAreaListItem>
        ...
        request={queryPreferredAreaList}
        columns={columns}
        rowSelection={ {} }
      ...
      >
        {currentRow?.id && (
          <ProDescriptions<PreferredAreaListItem>
            ...
            columns={columns as ProDescriptionsItemProps<PreferredAreaListItem>[]}
          />
        )}
      </Drawer>
    </PageContainer>
