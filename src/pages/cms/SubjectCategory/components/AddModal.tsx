...
const AddModal: React.FC<CreateFormProps> = (props) => {
  ...
  const { onSubmit, onCancel, open } = props;
  ...
  const handleFinish = (values: SubjectCategoryListItem) => {
    if (onSubmit) {
      onSubmit(values);
    }
  };
  ...
  const renderContent = () => {
    return (
      <>
        ...
        >
          <Input id="create-name" placeholder={'请输入专题分类名称!'} />
        </FormItem>
        <FormItem
          name="icon"
          label="分类图标"
  ...
  return (
    <Modal forceRender destroyOnClose title="新增" open={open} {...modalFooter}>
