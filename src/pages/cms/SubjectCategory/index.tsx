...
import {addSubjectCategory, querySubjectCategoryList, removeSubjectCategory, updateSubjectCategory, updateSubjectCategoryStatus} from './service';
 ...
 * @param fields
 */
const handleAdd = async (fields: SubjectCategoryListItem) => {
  const hide = message.loading('正在添加');
  try {
    await addSubjectCategory({...fields});
    hide();
 ...
 * @param fields
 */
const handleUpdate = async (fields: SubjectCategoryListItem) => {
  const hide = message.loading('正在更新');
  try {
    await updateSubjectCategory(fields);
    hide();
...
const handleRemove = async (ids: number[]) => {
  ...
  try {
    await removeSubjectCategory(ids);
    hide();
...
const handleStatus = async (ids: number[], status: number) => {
  ...
  try {
    await updateSubjectCategoryStatus({ids, status});
    hide();
...
return (
    <PageContainer>
      <ProTable<SubjectCategoryListItem>
        ...
        request={querySubjectCategoryList}
        columns={columns}
        rowSelection={ {} }
      ...
      >
        {currentRow?.id && (
          <ProDescriptions<SubjectCategoryListItem>
            ...
            columns={columns as ProDescriptionsItemProps<SubjectCategoryListItem>[]}
          />
        )}
      </Drawer>
    </PageContainer>
